...
if __name__ == '__main__':
  ...
  if len(sys.argv) != 3:
    if not MPI.COMM_WORLD.Get_rank():
      raise ValueError('tests takes only 2 input arguments: type demo_name. See help (-h) for options. ')
  else:
    try:
      demo = import_module('tests.scripts.' + sys.argv[1] + '.' + sys.argv[2] + '.' + sys.argv[2])
