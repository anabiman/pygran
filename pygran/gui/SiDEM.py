...
class MainWindow(wx.Frame):
    def __init__(self, parent, title, name):
        ...
        self.loadedVars = {}
        self.CWD = os.getcwd()
        self.read_pdb_options = {
            "Check_for_HETATM": True,
            "Debug": False,
            "T_number": 5,
        }
        ...
        self.Font = wx.Font(
            ...
            False,
            u"Consolas",
            encoding=wx.FONTENCODING_ISO8859_6,
        )
        self.nProcs = 1
    ...
    def MainMenu(self, language):
        ...
        self.OpenBtn = self.FileMenu.Append(
            id=wx.ID_OPEN, text="&Open\tCtrl-O", help="Open a script file"
        )
        self.SaveBtn = self.FileMenu.Append(
            id=wx.ID_CLOSE, text="&Save\tCtrl-S", help="Save output to file"
        )
        self.CWDBtn = self.FileMenu.Append(
            id=wx.ID_CLOSE,
            text="&Change Dir \tCtrl-D",
        ...
        self.AboutBtn = self.HelpMenu.Append(
            id=wx.ID_ABOUT, text="&About\tCtrl-A", help="About this program"
        )
        ...
        self.ConfBtn = self.ConfMenu.Append(
            id=wx.ID_SETUP, text="&Simulation\tCtrl-S", help="Setup Simulation"
        )
        self.Bind(event=wx.EVT_MENU, handler=self.onSimSetup, source=self.ConfBtn)

        self.setComputBtn = self.ConfMenu.Append(
            id=wx.ID_ANY, text="&Computation\tCtrl-C", help=""
        )
        self.Bind(event=wx.EVT_MENU, handler=self.onSetProcs, source=self.setComputBtn)

        self.setProcsBtn = self.ConfMenu.Append(
            id=wx.ID_ANY, text="&Hardware\tCtrl-H", help="Set number of cores"
        )
        self.Bind(event=wx.EVT_MENU, handler=self.onSetProcs, source=self.setProcsBtn)

        self.setEngBtn = self.ConfMenu.Append(
            id=wx.ID_ANY, text="&Engine\tCtrl-E", help="Set number of cores"
        )
        ...
        self.AtomAnBtn = self.AnaMenu.Append(
            id=wx.ID_ANY, text="&Atomistic\tAlt-Ctrl-A", help="Begin atomistic analysis"
        )
        self.CGAnBtn = self.AnaMenu.Append(
            id=wx.ID_ANY,
            text="&Coarse-Grained\tAlt-Ctrl-C",
            help="Begin macromolecular analysis",
        )
    ...
    def ToolBar(self):
        ...
        self.OpenBtn = self.ToolBar.AddLabelTool(
            wx.ID_ANY, "&Open", wx.Bitmap(iconsDir + "Load.png")
        )
        self.SaveBtn = self.ToolBar.AddLabelTool(
            wx.ID_ANY, "&Save", wx.Bitmap(iconsDir + "Save.png")
        )

        self.ToolBar.AddSeparator()

        self.runBtn = self.ToolBar.AddLabelTool(
            wx.ID_ANY, "&Start", wx.Bitmap(iconsDir + "Generate.png")
        )
        self.visBtn = self.ToolBar.AddLabelTool(
            wx.ID_ANY, "&Plot", wx.Bitmap(iconsDir + "Plot.png")
        )

        self.ToolBar.AddSeparator()

        self.ClrBtn = self.ToolBar.AddLabelTool(
            wx.ID_ANY, "&Clear", wx.Bitmap(iconsDir + "Clear.png")
        )
        self.QuitBtn = self.ToolBar.AddLabelTool(
            wx.ID_ANY, "&Quit", wx.Bitmap(iconsDir + "Quit.png")
        )
    ...
    def DisplayPanel(self):
        ...
        self.contents_txt = wx.TextCtrl(
            parent=self.Page_Output,
            size=(-1, -1),
    ...
    def OnTerminal_txt(self, event):
        ...
        process = subprocess.Popen(
            [self.command_txt.Value], stdout=subprocess.PIPE, shell=True
        )
    ...
    def LoadedPanel(self):
        ...
        self.LoadedPanel = wx.Panel(
            self, -1, style=wx.TAB_TRAVERSAL | wx.BORDER, name="Loaded Panel"
        )
        self.LoadedBox = wx.BoxSizer(wx.HORIZONTAL)

        # upper_box1 text stuff
        self.ScriptFileTxt = wx.TextCtrl(
            self.LoadedPanel,
            value="Script file:",
            style=wx.TE_READONLY | wx.TRANSPARENT_WINDOW | wx.ALIGN_CENTER_HORIZONTAL,
        )

        self.LoadedScript = wx.TextCtrl(
            self.LoadedPanel, value="None", style=wx.ALIGN_CENTER_HORIZONTAL
        )
    ...
    def InputPanel(self):
        ...
        self.InputPanel = wx.Panel(
            self, -1, style=wx.TAB_TRAVERSAL | wx.BORDER, name="Input Panel"
        )
        self.InputBox = wx.BoxSizer(wx.HORIZONTAL)

        self.InputName = wx.TextCtrl(
            self.InputPanel,
            value="Input command:",
            style=wx.TE_READONLY | wx.TRANSPARENT_WINDOW | wx.ALIGN_CENTER_HORIZONTAL,
        )

        self.InputTxt = wx.TextCtrl(
            self.InputPanel,
            value="",
        ...
        if len(command.split()) == 1:
            ...
            if method == "plot":
                    ...
                    try:
                        if var == "mesh":
                            ...
                            if var == "mesh and particles":
                                if var not in self.loadedVars:
                                    self.UpdateDisplayPanel(
                                        "Error: no particles loaded"
                                    )
                                else:
                                    try:
                                        visualize.visualize(
                                            meshFname=self.loadedVars["mesh"],
                                            dumpFname=self.loadedVars[
                                                "mesh and particles"
                                            ],
                                        )
                                    except:
                                        self.UpdateDisplayPanel(
                                            "Plot failed with unexpected error: {}".format(
                                                sys.exc_info()[0]
                                            )
                                        )
                            else:
                ...
                if self.__selEngine__:
                    ...
                    self.UpdateDisplayPanel(
                        "No engine selected. Make sure an available DEM engine is installed."
                    )
            elif method == "unix":
                try:
...
Suite 330, Boston, MA  02111-1307  USA"""
    ...
    def onImpMeshBtn(self, event):
        ...
        if dlg.ShowModal() == wx.ID_OK:
            ...
            self.UpdateDisplayPanel(
                "Loaded mesh from {}".format(self.loadedVars["mesh"])
            )
    ...
    def onImpTrajBtn(self, event):
        ...
        if dlg.ShowModal() == wx.ID_OK:
            ...
            self.UpdateDisplayPanel(
                "Loaded trajectory from {}".format(
                    self.loadedVars["mesh and particles"]
                )
            )
    ...
    def OnOpen(self, event):
        ...
        dlg = wx.FileDialog(
            ...
            defaultDir=self.CWD,
            defaultFile="",
            style=wx.FD_FILE_MUST_EXIST | wx.FD_OPEN | wx.FD_PREVIEW,
            wildcard=wildcard,
        )
        ...
        if dlg.ShowModal() == wx.ID_OK:
            ...
            if self.loadedScript.split(".")[1] == "py":
                ...
                self.UpdateDisplayPanel(
                    "Loaded input script file: {}".format(self.loadedScript)
                )
    ...
    def OnSave(self, event):
        ...
        if dlg.ShowModal() == wx.ID_OK:
            try:
                ...
                self.UpdateDisplayPanel(
                    "Attempting to save file {} ...".format(pdb_file_name)
                )
                fp.write(self.Generated_PDB)

                self.UpdateDisplayPanel(
                    "Success! Coordinates file {} written to {}".format(
                        pdb_file_name, cdir
                    )
                )
                fp.close()
                ...
                raise ValueError(
                    "Could not save output file {} to {}".format(pdb_file_name, cdir)
                )
        dlg.Destroy()
    ...
    def onVisualize(self, event):
        ...
        try:
            ...
            self.UpdateDisplayPanel(
                "No visualization software found. Make sure ovito is properly installed on your system."
            )

    def OnAA(self, event):
        Analysis = event.GetSelection()
        diag = wx.Dialog(
            self, size=(800, 600), title=Analysis, style=wx.DIALOG_NO_PARENT
        )
    ...
    def OnGen(self, event):
        ...
        if self.loadedScript:
            try:
                ...
                # ver hackish!!!
                self.loadedScriptPy.pDict["traj"]["output"] = (
                    wdir + "/" + sorted(glob.glob("out-*"), key=os.path.getmtime)[-1]
                )
    ...
    def OnGetIndices(self, event):
        dlg = wx.Dialog(
            parent=self, title="I am modal, close me first to get to main frame"
        )
        dlg.ShowModal()
    ...
    def OnSelDir(self, event):
        ...
        if dlg.ShowModal() == wx.ID_OK:
            ...
            self.UpdateDisplayPanel(
                "Changed current working directory to {}".format(self.CWD)
            )
    ...
    def OnAtomAnBtn(self, event):
        ...
        dlg = wx.Dialog(
            ...
            style=wx.DEFAULT_DIALOG_STYLE | wx.BORDER_SUNKEN,
        )
        dlg.ShowModal()
    ...
    def onSetProcs(self, event):
        ...
        if dlg.ShowModal() == wx.ID_OK:
            ...
            if type(self.nProcs) is not int:
                self.UpdateDisplayPanel(
                    "Error. Number of processors must be an integer."
                )
                self.nProcs = 1
            else:
    ...
    def onSetEng(self, event):
        ...
        if dlg.ShowModal() == wx.ID_OK:
            ...
            self.UpdateDisplayPanel(
                "Importing PyGran.Simulator.engine." + self.__selEngine__
            )
            self._module = import_module(
                "PyGran.Simulator.engine_" + self.__selEngine__
            )
    ...
    def OnRCSB(self, event):
        ...
        if dlg.ShowModal() == wx.ID_OK:
            ...
            self.url = "http://www.rcsb.org/pdb/download/downloadFile.do?fileFormat=pdb&compression=NO&structureId={}".format(
                self.PDB_ID
            )
            ...
            try:
                ...
                self.UpdateDisplayPanel(
                    "Failed to download file. File does not exist or connection timed out."
                )
                Success = False
    ...
    def CustomDiag(self, event):
        ...
        PDB_ID_RE_txt = wx.TextCtrl(
            parent=self.RCSB_dlg, style=wx.TE_READONLY, value="PDB ID:"
        )
        ...
        self.RCSB_dlg.Bind(
            event=wx.EVT_BUTTON, handler=self.OnRCSBSubmit, source=SubmitBtn
        )
        self.PDB_ID_txt.Bind(wx.EVT_KEY_DOWN, self.OnRCSBSubmit)

        self.RCSB_dlg.Bind(
            event=wx.EVT_BUTTON, handler=self.OnRCSBCancel, source=CancelBtn
        )
    ...
    def OnOlig(self, event):
        ...
        panel_tmp = wx.Panel(
            parent=frame_tmp,
            id=-1,
            style=wx.TAB_TRAVERSAL | wx.BORDER,
            name="Oligomer indices generator panel",
        )
        ...
        oligo_txt = wx.TextCtrl(
            panel_tmp,
            value="Oligomer number:",
    ...
    def CheckForHETATM(self, event):
        ...
        if self.cb.GetValue():
            ...
            self.UpdateDisplayPanel(
                "Hetero atoms will now be taken into account when reading pdb files(s)."
            )
        else:
            self.read_pdb_options["Check_for_HETATM"] = False
            self.UpdateDisplayPanel(
                "Hetero atoms will NOT be taken into account when reading pdb file(s)."
            )
