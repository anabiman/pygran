...
voidFraction = [Inter.points.voidfraction[Inter.points.voidfraction < 1].mean() for ts in Bed]
#voidFraction = [Inter.points.voidfraction.mean() for ts in Bed]

# Properties of the fluid and granules
f_density, p_density, kin_vis, diameter = 10.0, 2000.0, 1.5e-4, Bed.Particles.radius.mean() * 2
...
print('umf = ', + 1e3 * diameter**2 * (p_density - f_density) * 9.81 * voidFraction**3 / (150 * dyn_vis *  (1 - voidFraction)))

# Compute fluid superficial velocity
fsvel = fvel * voidFraction

# Compute the pressure drop analytically (with Ergun eq)
term1 = 150.0 * dyn_vis / diameter**2 * fsvel * (1 - voidFraction)**2 / voidFraction**3
term2 = 1.75 * f_density / diameter * fsvel * (1 - voidFraction) / voidFraction**3 * fsvel
