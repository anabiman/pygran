...
if __name__ == "__main__":
    ...
    if sys.argv[1] == "-h" or sys.argv[1] == "--help":
        ...
        if not MPI.COMM_WORLD.Get_rank():
            ...
            for dtype in actual_dirs:
                ...
                py_dirs = [
                    py_dir.split(dtype + "/")[-1]
                    for py_dir in py_dirs
                    if os.path.isdir(py_dir)
                ]
                print(
                    "{}: ".format(dtype), ("{}, " * len(py_dirs)).format(*py_dirs)[:-2]
    ...
    if len(sys.argv) != 3:
        if not MPI.COMM_WORLD.Get_rank():
            raise ValueError(
                "tests takes only 2 input arguments: type demo_name. See help (-h) for options. "
            )
    else:
        try:
            demo = import_module(
                "tests.scripts." + sys.argv[1] + "." + sys.argv[2] + "." + sys.argv[2]
            )
            ...
            if hasattr(demo, "params"):
                if "mesh" in demo.params:
                    for key in demo.params["mesh"]:
                        demo.params["mesh"][key]["file"] = os.path.join(
                            sdir,
                            "scripts",
                            sys.argv[1],
                            sys.argv[2],
                            demo.params["mesh"][key]["file"],
                        )
